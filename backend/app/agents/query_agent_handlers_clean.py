...
**Note:** Impact:** Missing data can affect analysis accuracy. Check the **Data Quality** tab for detailed recommendations on handling these gaps."""
        else:
...
def _handle_insights(self, question, data_summary, context, entities):
    ...
    if insights:
        ...
        return f"""**Note:** Key Insights Discovered ({len(insights)} total):**
...
def _handle_recommendations(self, question, data_summary, context, entities):
    ...
    if recommendations:
        ...
        if high_priority:
            ...
            return f"""**Note:** Recommendations ({len(recommendations)} total):**
...
• **Note:** Recommendations:** {len(recommendations)} available
